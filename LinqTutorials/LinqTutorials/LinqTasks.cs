...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            var result = Emps
                .Where(empOut => empOut.Salary == Emps
                    .Max(empIn => empIn.Salary));
            return result;
        }
        ...
        public static IEnumerable<object> Task10()
        {
            ...
            IEnumerable<object> result2 = Emps.Select(_ => new
            {
                Ename = "Brak wartości",
                Job = "",
                HireDate = ""
        ...
        public static IEnumerable<object> Task11()
        {
            IEnumerable<object> result = Emps
                ...
                .GroupBy(emp => emp.Dname)
                .Select(gr => new
                {
                    name = gr.Key,
                    numOfEmployees = gr.Count()
                })
                .Where(row => row.numOfEmployees > 1);
            return result;
        }
        ...
        {
            var allDepartments = Depts.Select(dept => new
            {
                dept.Deptno,
                dept.Dname,
                dept.Loc
            });
            var departmentsWithEmployees = Emps
                .Join(Depts, emp => emp.Deptno, dept => dept.Deptno, (_, dept) => new
                {
                    dept.Deptno,
                    dept.Dname,
                    dept.Loc
                });
            var departmentsWithNoEmployees = allDepartments.Except(departmentsWithEmployees);

            var departmentsWithFiveEmployees = Emps
                .Join(Depts, emp => emp.Deptno, dept => dept.Deptno, (_, dept) => new
                {
                    dept.Deptno,
                    dept.Dname,
            ...
            var resultDepartments = departmentsWithNoEmployees
                .Union(departmentsWithFiveEmployees);

            return resultDepartments.Select(dept => new Dept
            {
                Deptno = dept.Deptno,
                Dname = dept.Dname,
                Loc = dept.Loc
